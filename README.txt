...
### 9.6
- 修正了一个bug，对文本长、宽小于10像素的限制去掉，有些情况他会过度的滤掉正样本

### 12.3
- 解决了原来代码中的疑问（画平行四边形的时候，按照左下角进行计算，会切掉一部分文本框），实际是左上角，不是左下角，这样就保证画出来的四边形不会切到文本框
- 修改了之前icdar-776行的注释（画出来两个平行四边形，取最大的一个），改为取最小的平行四边形，论文中提到取文本框的最小的外接旋转矩形框
- ICDA2015数据集中存在三点共线的图片标签，在代码icdar添加了判断三点共线的函数，在第一步加载图片的时候，把三点共线的样本直接舍弃
- 用ICDA2015训练的结果F1还是不收敛，断定代码应该有问题
- 研究了损失函数和F1的计算过程，感觉没问题，后续有时间继续研究
